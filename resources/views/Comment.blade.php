        ...
        <div class="max-w-sm mx-auto px-4">
          ...
          <form method="POST" action="/add-comment" class="mb-6">
            ...
            <div class="py-2 px-4 mb-4 bg-black rounded-lg rounded-t-lg border border-gray-200">
              ...
              <textarea id="comment" rows="6" name="comment" class="bg-black px-0 w-full text-sm text-white border-0 focus:ring-0 focus:outline-none" placeholder="Write a comment..." required></textarea>
            </div>
            <button type="submit" class="inline-flex items-center py-2.5 px-4 border border-white rounded-lg text-xs font-medium text-center text-white bg-primary-700 focus:ring-4 focus:ring-primary-200 hover:bg-primary-800">
              Post comment
            </button>
          </form>
          @foreach ($comment as $item)
          <article class="p-6 text-base bg-black rounded-lg mb-6">
            <footer class="flex justify-between items-center mb-2">
              <div class="flex items-center">
                <img class="mr-2 w-10 h-10 rounded-full" src="https://3.bp.blogspot.com/-bPMEDt2hB0Q/ULlbJeDCjYI/AAAAAAAAE08/aQjEWu7oJ5E/s1600/433.jpg" alt="Profile Picture">
                <div>
                  <p class="text-sm text-white font-semibold">{{ $item->user->name }}</p>
                  <p class="text-sm text-gray-600">{{ \Carbon\Carbon::parse($item->created_at)->format('d F Y') }}</p>
                </div>
              </div>
            </footer>
            ...
            <div class="flex flex-col items-center mt-4 space-x-4">
              <form class="w-full hidden" method="POST" action="/sub-comment">
                ...
                <textarea id="comment" rows="6" name="sub_comment" class="bg-dark p-3 rounded-md w-full text-sm text-black border-0 focus:ring-0 focus:outline-none" placeholder="Write a comment..." required></textarea>
                <button type="submit" class="px-2 py-1 mt-3 border border-white rounded-md">Submit</button>
              </form>
              <button id="replay" type="button" class="flex items-center text-sm text-gray-500 hover:underline font-medium">
                <svg class="mr-1.5 w-3.5 h-3.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 18">
                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5h5M5 8h2m6-3h2m-5 3h6m2-7H2a1 1 0 0 0-1 1v9a1 1 0 0 0 1 1h3v5l5-5h8a1
                  1  1  0  0  0  1-1V2a1 1  0  0  0-1-1Z"/>
                </svg>
                Reply
              </button>
            </div>
            ...
            <div class="mt-5 ml-10">
              <div class="flex items-center">
                <img class="mr-2 w-10 h-10 rounded-full" src="https://3.bp.blogspot.com/-bPMEDt2hB0Q/ULlbJeDCjYI/AAAAAAAAE08/aQjEWu7oJ5E/s1600/433.jpg" alt="Profile Picture">
                <div>
                  <p class="text-sm text-white font-semibold">{{ $subcomment->user->name }}</p>
                  <p class="text-sm text-gray-400">{{ $subcomment->created_at->toFormattedDateString() }}</p>
                </div>
              </div>
